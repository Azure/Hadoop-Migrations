...
Azure has several landing targets for Apache HBase. Depending on requirements and product features, customers can choose between Azure IaaS, HDInsight HBase or Cosmos DB (SQL API).  
...
&nbsp; | Consumption | How is data stored in HBase used? APIs or a compute engine such as HDInsight Spark or Databricks Spark?
...
The migration guide to the Cosmos DB SQL API has been moved to the official Cosmos DB documentation.
Please refer to [Migrate data from Apache HBase to Azure Cosmos DB SQL API account](https://docs.microsoft.com/azure/cosmos-db/sql/migrate-hbase-to-cosmos-db).
